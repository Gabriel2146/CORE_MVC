...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        if (this.isEdit) {
          await axios.put(`https://<TU_BACKEND_RENDER_URL>/api/users/trainer-training-plans/${this.$route.params.id}/`, payload, {
            headers: { Authorization: 'Bearer ' + localStorage.getItem('access_token') }
          })
          this.success = 'Plan actualizado correctamente.'
        } else {
          await axios.post('https://<TU_BACKEND_RENDER_URL>/api/users/trainer-training-plans/', payload, {
            headers: { Authorization: 'Bearer ' + localStorage.getItem('access_token') }
          })
          this.success = 'Plan creado correctamente.'
          this.form = { athlete: '', name: '', duration: 4, sessions: '' }
        }
      } catch (e) {
...
<style scoped>
.trainer-training-plan-form-container {
  max-width: 600px;
  margin: 40px auto;
  padding: 24px;
